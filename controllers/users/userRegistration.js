...
exports.login = async (req, res, next) => {
  try {
    ...
    if (isExists.block == true)
      return res
        .status(400)
        .json({ error: "You are blocked please contact with admin" });
    const isAuthUser = await decryptPassword(password, isExists.password);
    if (isAuthUser) {
      let { password, ...data } = isExists._doc;
      const jwtToken = jwt.sign({ user: data }, process.env.SECRET, {
        expiresIn: "24h",
      });
  ...
  try {
    ...
    if (isExists.block == true)
      return res
        .status(400)
        .json({ error: "You are blocked please contact with admin" });
    if (Number(isExists.role) != 1)
      return res
        .status(400)
        .json({ error: "Only Admin can access this panel" });
    const isAuthUser = await decryptPassword(password, isExists.password);
    if (isAuthUser) {
      let { password, ...data } = isExists._doc;
      const jwtToken = jwt.sign({ user: data }, process.env.SECRET, {
...
exports.adminsignup = async (req, res, next) => {
  try {
    ...
    if (isAdminExist)
      return res.status(400).json({ error: "Multiple admin not allowed" });
    let isExists = await User.findOne({ $or: [{ email }, { username }] });
...
exports.signup = async (req, res, next) => {
  try {
    ...
    if (!isRefferalExists)
      return res.status(400).json({ error: "Refferal Code invalid" });
    ...
    let result = await UserSave.save();
    let addRefferalTo = await User.findOneAndUpdate(
      { userId: refferalCode },
      { $push: { refferedTo: userId } }
    );
    const { password, ...data } = result._doc;
    return res.status(200).json({ message: "Registered successfully", data });
  } catch (error) {
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    if (update) {
      ...
      return res
        .status(200)
        .json({ message: "Please check your mail to change your password" });
    } else {
...
exports.reset = async (req, res) => {
  try {
    ...
    if (!isValidRequest)
      return res.status(400).json({ error: "Invalid Reset password request" });
...
exports.blockUser = async (req, res, next) => {
  try {
    ...
    if (!user)
      return res
        .status(400)
        .json({ error: "Please provide a authenticate token" });
    if (!userId)
      return res.status(400).json({ error: "Please provide a userId" });
    if (!isBlock)
      return res.status(400).json({ error: "Please provide a block status" });
    let isExists = await User.findOne({ userId });
    if (!isExists) return res.status(400).json({ error: "User Not Found" });
...
exports.editUser = async (req, res, next) => {
  try {
    let user = req.user.user;
    let { bankName, accounttype, accountNumber, IFSCCode, UsdtAddress } =
      req.body;
    ...
    if (!req.files.photo[0].originalname.match(/\.(jpg|jpeg|png|doc|exl)$/)) {
...
const filterData = async (startDate, endDate, keywords) => {
  ...
  if (startDate && endDate && keywords) {
    ...
    keywords = keywords === "true2" ? true : keywords;
    query = {
      $or: [{ block: { $eq: keywords } }, { isInvested: { $eq: keywords } }],
    };
  } else if (!keywords && startDate && endDate) {
    const sdate = new Date(startDate);
    const edate = new Date(endDate);
...
exports.sendMailOtp = async (req, res, next) => {
  try {
    ...
    if (isExists) {
      let update = await OTP.updateOne(
        { email: { $eq: email } },
        { $set: { otp: otp } }
      );
      let { message } = require("../../templates/otp.js");
